...
let db
const select =
  'SELECT COUNT(*) AS hits, plugin FROM graphdata WHERE timestamp >= ? AND timestamp < ? GROUP BY plugin'
let insert
let plugins = {}
let config
...
function createTable() {
  db.exec(
    'CREATE TABLE IF NOT EXISTS graphdata (timestamp INTEGER NOT NULL, plugin TEXT NOT NULL)',
...
exports.disconnect = function (next, connection) {
  if (!connection.current_line) {
    ...
    return this.deny(next, connection, [
      constants.DENY,
      'random disconnect',
      'disconnect_early',
    ])
